...
print(f"Starting training. Target loss: {target_loss}")
print(f"Saving snapshots every {snapshot_interval} steps")

# Training loop - continue until target loss is reached
while True:
...
print(f'\nLoading final model from {final_model_path} for prediction...')
checkpoint = torch.load(final_model_path, map_location=device)
model.load_state_dict(checkpoint['model_state_dict'])
model.eval()
print(f'Model loaded. Final loss was: {checkpoint["loss"]:.6f}')

# Prepare sample input for prediction
print('\nGenerating predictions on sample input...')
enc = tiktoken.get_encoding('gpt2')

# Create a sample input (you can modify this to use your own sample)
sample_text = "The quick brown fox"
sample_tokens = enc.encode(sample_text)
x = torch.tensor([sample_tokens], dtype=torch.long, device=device)

print(f"Sample input: '{sample_text}'")
print(f"Generating {num_return_sequences} sequences of max length {max_length}...")
...
for i in range(min(num_return_sequences, x.size(0))):
